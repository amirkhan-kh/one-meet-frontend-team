...
const PaymentHistory = () => {
  ...
  useEffect(() => {
    if (data?.content?.[0]?.id) {
      dispatch(fetchPaymentsByCompany(data.content[0].id));
    }
  }, [data, dispatch]);
          ...
          <TableBody>
            {data?.content?.map((item, index) => (
              <TableRow key={item.id || index}>
                ...
                <TableCell className={`translate-x-2`}>{item.plan?.amount || "-"}</TableCell>
                <TableCell className={`translate-x-2`}>{item.plan?.credits || "-"}</TableCell>
                <TableCell className={`translate-x-2`}>{item.plan?.currency || "-"}</TableCell>
                <TableCell>
                  {item.createdAt ? (
                    <>
                      <span>
