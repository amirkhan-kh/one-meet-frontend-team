...
const PaymentHistory = () => {
  ...
  useEffect(() => {
    if (dataPlan?.id) {
      dispatch(fetchCompanyByOwnerId(dataPlan.id));
    }
  }, [dataPlan, dispatch]);
  ...
  return (
    <div className="shadow-md rounded-md mb-10 bg-white translate-y-5">
      <div className="">
        <p className=" text-[19px] font-bold text-white animated-gradient-card p-4 rounded-tl-md rounded-tr-md">
          Payment History
        </p>
      </div>
      <div className="">
        <Table>
          <TableHeader
            className={`animated-gradient-card text-white hover:bg-transparent focus:ring-0 `}
          >
            <TableRow
              className={`hover:bg-transparent focus:ring-0 focus:outline-none border-none shadow-none p-4`}
            >
