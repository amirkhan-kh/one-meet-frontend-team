...
const PaymentsMethod = () => {
  ...
  const companyData = useSelector((state) => state.companyByOwner);
...
useEffect(() => {
  ...
  const handlePayment = (planId) => {
    const companyId = companyData?.id.id;
    ...
    dispatch(createPayment({ companyId, planId }))
      .then((action) => {
         const res = action.payload;
          console.log("Result:", res.data?.sessionUrl);
        if (res) {
            ...
            {dataPlan.map((item) => (
              ...
              >
                <p className="font-semibold mb-4 flex items-center justify-between border-b border-gray-300 pb-2 text-[22px]">
                  ...
                  <button className="cursor-pointer text-[12px] bg-[rgba(0,0,0,0.1)] hover:bg-[rgba(108,107,107,0.1)] py-1.5 px-3 rounded-2xl">
                    Choose plan
                  </button>
                </p>
