...
const result = dataPlan.map((item) => item.id)
console.log(result);
...
useEffect(() => {
  dispatch(fetchUserProfile()); // 🚀 MUHIM: user ID ni olish uchun zarur
}, [dispatch]);

useEffect(() => {
  if (data?.id) {
    dispatch(fetchCompanyByOwnerId(data.id)); // ✅ companyId olish
  }
}, [data, dispatch]);
