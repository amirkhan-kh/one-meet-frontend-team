    ...
    <header className="ai-header">
      <div className="flex items-center justify-between px-6 py-3 w-full">
        ...
        <div className="hidden sm:block lg:hidden">
          <nav
            className={`ai-nav ${menuOpen ? "show" : ""} translate-y-0 h-[208px]`}
          >
            <div className="flex gap-4 w-full pr-2">
              <div className=" w-[200px] py-2">
                <ul className="text-left pl-[20px]  h-[160px] py-1.5">
                  {currentLinks.map((item, i) => (
                    <NavLink
                      key={i}
                      to={item.pathName}
                      className="underline-hover bg-cyan-100"
                    >
                      <li
                        className="text-[15px] font-semibold py-1.5"
