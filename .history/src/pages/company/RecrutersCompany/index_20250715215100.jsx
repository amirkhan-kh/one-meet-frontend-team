...
export const RecruitersCompany = () => {
  ...
  const approved = recruiters.filter((r) => r.approvedByCompany && r.approvedByRecruiter);
  const pendingByCompany = recruiters.filter((r) => r.approvedByRecruiter && !r.approvedByCompany);
  const pendingByRecruiter = recruiters.filter((r) => !r.approvedByCompany && !r.approvedByRecruiter);
    ...
    return (
      <div className="recruiter-section shadow-sm border-l-[4px] border-[#2b43d4] mt-4">
        ...
        <div className="flex items-center justify-between">
          <h2 className="translate-x-3">{title}</h2>
        </div>
        {data.length === 0 ? (
          <p className="empty-row translate-x-3">No recruiters found</p>
        ) : (
          <>
            <Table>
              <TableHeader>
                <TableRow className="text-black bg-[#f4f5fd] hover:bg-[#f4f5fd] ">
                  <TableHead className={`translate-x-3.5`}>Name</TableHead>
                  <TableHead className={`translate-x-3.5`}>Position</TableHead>
                  <TableHead className={`translate-x-3.5`}>Joined At</TableHead>
              ...
              <TableBody>
                {currentItems.map((r) => (
                  <React.Fragment key={r.id}>
                    <TableRow
                      className={`bg-white ${selectedRecruiterId === r.id ? "active" : ""}`}
                      onClick={() =>
                        setSelectedRecruiterId((prev) => (prev === r.id ? null : r.id))
                      }
                    >
