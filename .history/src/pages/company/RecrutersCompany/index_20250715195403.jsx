...
export const RecruitersCompany = () => {
  ...
  const renderTable = (title, data, showAccept) => (
    <div className="bg-white rounded-md my-8 py-4   border-l-[4px] border-[#2b43d4] ">
      <div className="flex items-center justify-between ">
        <h2 className="ms-4 my-3 text-[20px] font-semibold">{title}</h2>
      </div>
      {data.length === 0 ? (
        <p className="empty-row ms-4">No recruiters found</p>
      ) : (
        <Table className="">
          <TableHeader className={` text-black hover:bg-[#f4f5fd] focus:ring-0 bg-[#f4f5fd] `}>
            <TableRow className={`translate-x-2.5`}>
          ...
          <TableBody>
            {data.map((r) => (
              <React.Fragment key={r.id}>
                <TableRow
                  className={` className={`translate-x-2.5`} ${
