...
export default function CompanyCreateRecruiterModal({ companyId, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setOpen(false);
      setForm({ email: "", firstName: "",     companyName: "",  position: "" });
      onSuccess?.();
    } catch (err) {
      console.error(err);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild className="bg-[#0823d4bc] hover:bg-[#0823d4bc] text-white px-3 py-1 rounded-sm">
        <Button >
          Add Recruiter
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle className={`mb-3`}>Create Recruiter</DialogTitle>
          </DialogHeader>
