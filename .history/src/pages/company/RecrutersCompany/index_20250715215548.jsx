...
export const RecruitersCompany = () => {
  ...
  const renderTable = (title, data, showAccept, key) => {
    ...
    return (
      <div className="recruiter-section shadow-sm border-l-[4px] border-[#2b43d4] mt-4">
        ...
        {data.length === 0 ? (
          <p className="empty-row translate-x-5">No recruiters found</p>
        ) : (
          <>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {currentItems.map((r) => (
                  <React.Fragment key={r.id}>
                    <TableRow
                      className={`bg-white hover:bg-white translate-x-3 ${selectedRecruiterId === r.id ? "active" : ""}`}
                      onClick={() =>
  ...
  return (
    <div className="p-3">
      {renderTable(
        <div className=" gap-3 justify-between mb-4">
