...
import { MdCheckCircleOutline, MdAccessTime, MdOpenInBrowser } from "react-icons/md";
...
export const RecruitersCompany = () => {
  ...
  if (isLoading)
    return (
      <div className="p-3  rounded-md border-l-[4px] border-[#2b43d4]">
  ...
  return (
    <div className="recruiter-wrapper p-3">
      {renderTable(
        <> <MdCheckCircleOutline size={24} className="inline mr-[2px] text-green-400"/> Active Recruiters </>,
        approved, false
      )}
      {renderTable(
        <> <MdAccessTime size={24} className="inline mr-[2px] text-blue-400"/> Pending Approval by Company </>,
        pendingByCompany, true
      )}
      {renderTable(
        <><MdOpenInBrowser size={24}  className="inline mr-[2px] text-sky-800"/> Join Requests from Recruiters </>,
        pendingByRecruiter, true
      )}
    </div>
  );
};
