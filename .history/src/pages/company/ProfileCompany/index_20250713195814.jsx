...
export const ProfileCompany = () => {
  ...
  const handleSave = async () => {
    const token = localStorage.getItem("accessToken");
    try {
      const res = await fetch(`https://api.onemeet.app/company/update/${company.id}`, {
        method: "PUT",
        headers: {
  ...
  return (
    <div className="px-4 lg:px-40 py-8">
      <h3 className="text-2xl font-semibold mb-4">Company Profile Management</h3>
      <p className="text-[15px] mb-8">
        Update your company information to keep your profile accurate and up-to-date.
      </p>
      ...
      <div className="flex flex-col md:flex-row gap-2 sm:gap-5 w-full">
        <div className="flex flex-col sm:flex-row md:flex-col w-full md:w-[30%] gap-2 sm:gap-6 md:gap-0">
          <div className="shadow px-3 sm:px-6 py-3 sm:py-8 bg-white rounded-md mb-4 sm:mb-10 w-full">
            ...
            <div className="shadow rounded-sm w-full h-40">
              {loading || error ? (
                ...
                <img src={logo} alt="logo" className="object-contain h-full rounded-md w-full" />
              ) : (
                <div className="text-center text-sm text-gray-400 pt-12">No Logo</div>
              )}
            </div>
            <ul>
              <li className="text-[12px] text-gray-600 my-1 font-light">Company Name</li>
              <span>
                {loading || error ? <Skeleton className="w-1/2 h-4" /> : company?.name || "Not provided"}
              </span>
          ...
          <div className="shadow px-3 sm:px-6 py-3 sm:py-8 bg-white rounded-md mb-4 sm:mb-10 w-full flex flex-col justify-between gap-4">
            <h3 className="font-semibold text-[17px]">Subscription Information</h3>
            <div>
              <p>
                Current Plan: {loading || error ? <Skeleton className="inline-block w-20 h-4" /> : data?.plan || "Free"}
              </p>
              <p>
                Payment Status: {loading || error ? <Skeleton className="inline-block w-24 h-4" /> : data?.paymentStatus || "Pending"}
              </p>
            </div>
            <Button className="text-white bg-gradient-to-r from-[#3b45d6] to-[#611bd6] hover:text-[#d0cccc] text-[12px]" variant="outline">
              ...
              <div className="flex flex-col items-center">
                <div className="w-24 h-24 bg-white border border-gray-300 rounded-md flex items-center justify-center overflow-hidden">
                  {loading || error ? (
                    ...
                    <img src={logo} alt="Company Logo" className="object-contain h-full" />
                  ) : (
