...
const summarize = async () => {
  try {
    ...
    const isCompleted = response.headers.get('X-Interview-Completed') === 'true';

    if (!response.ok) {
      const errorText = await response.text(); // Only read if needed
      console.error('❌ Summarize failed (non-200)', { status: response.status, errorText });
      navigate('/candidate');
      return;
    }
  ...
  useEffect(() => {
    const fetchAudio = async () => {
      try {
        ...
        const response = await fetch(`https://api.onemeet.app/interview/candidate/start/${interviewId}`, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${AOS_TOKEN}`
          }
      ...
      try {
        ...
        mediaRecorder.ondataavailable = async (event) => {
          if (event.data.size > 0 && !isMutedRef.current) {
            ...
            try {
              await fetch(`https://api.onemeet.app/interview/candidate/chunks/${interviewId}`, {
                method: 'POST',
  ...
  const toggleMute = () => {
    setIsMuted((prev) => {
      ...
      console.log(next ? '🎙️ Muted: Chunks will NOT be sent' : '🎙️ Unmuted: Chunks will be sent');
  ...
  return (
    <div className="interview-container">
      ...
      {audioUrl && (
        <audio ref={audioRef} autoPlay src={audioUrl} style={{ display: 'none' }} />
      )}
      ...
      {mutedMessage && (
        <div className="muted-info-banner">
          {mutedMessage}
        </div>
      )}
      ...
      {showExitConfirm && (
        <div className="exit-confirm-modal">
          <div className="exit-dialog">
            ...
            <p>
              If you leave now, the interview will be <strong>marked as incomplete</strong>, and recruiters will not be able to view your results.
            </p>
            <div className="exit-buttons">
              <button className="cancel-btn" onClick={() => setShowExitConfirm(false)}>Cancel</button>
              <button className="leave-btn" onClick={() => navigate('/candidate')}>Yes, Leave Interview</button>
            </div>
          </div>
        </div>
      )}
