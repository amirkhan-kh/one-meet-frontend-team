...
import { BarChart3, HelpCircle, Home, Menu, User } from 'lucide-react'
import { useEffect, useState } from 'react'
import { Link, useLocation, useNavigate } from 'react-router-dom' // React Routerdan kerakli hooklar
...
const NavLink = ({ to, onClick, className, children }) => {
	...
	return (
		<Link
			to={to} // Href orqali sahifaga yo'naltiradi
			onClick={e => {
				e.preventDefault() // Sahifani qayta yuklamaslik uchun
				navigate(to) // Sahifaga navigatsiya qilish
				onClick?.() // onClick handler, agar bo'lsa
			}}
			className={
				typeof className === 'function'
					? className({ isActive }) // Agar className function bo'lsa, u holda dynamic class qo'shadi
					: className
			}
		>
			{typeof children === 'function'
				? children({ isActive }) // Agar children dynamic bo'lsa, aktiv holatni aniqlash
				: children}
		</Link>
	)
}
...
export const MobileNavigation = () => {
	...
	const staticLinks = [
		{ pathName: 'interviews-rec', navName: 'Dashboard', icon: Home },
		{ pathName: '/contact-support', navName: 'Help', icon: HelpCircle },
		{ pathName: 'profile-recruiter', navName: 'Profile', icon: User },
		{ pathName: 'usage', navName: 'Usage', icon: BarChart3 },
	]
