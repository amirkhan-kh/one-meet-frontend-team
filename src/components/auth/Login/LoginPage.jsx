	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (data.success) {
				...
				// 4. Profile mavjudligini tekshirish
				try {
					const profileRes = await axios.get('https://api.onemeet.app/user/me', {
						headers: {
							Authorization: `Bearer ${accessToken}`,
						},
					});
					...
					if (profileRes.data?.success) {
						if(profileRes.data.data.profileCompleted){
							window.location.href = `/${authRole.toLowerCase()}`;
						} else {
							window.location.href = `/update-profile/${authRole.toLowerCase()}`;
						}
						
					} else {
						window.location.href = `/complete-profile/${authRole.toLowerCase()}`;
					}
				} catch (err) {
					window.location.href = `/complete-profile/${authRole.toLowerCase()}`;
				}
